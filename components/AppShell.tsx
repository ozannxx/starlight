...
import {
  AGENDA_KEY, TASKS_KEY, APPS_KEY, readJSON, addDays, todayISO, daysBetween,
  type AgendaEvent, type Task, type Application,
} from "@/lib/storage";
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    readJSON<Task[]>(TASKS_KEY, []).filter((t) => t.column !== "done" && t.due && t.due <= tomorrow)
      .forEach((t) => list.push({ text: `À rendre : ${t.title}`, sub: t.due! < today ? "⚠️ En retard" : "Échéance proche", href: "/tasks" }));
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div className="relative">
                <button onClick={() => setBellOpen((v) => !v)} aria-label="Notifications"
                  ...
                  <Bell size={18} />
                  {notifs.length > 0 && <span className="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-accent px-1 text-[10px] font-bold text-white ring-2 ring-white/80">{notifs.length}</span>}
                </button>
                {bellOpen && (
                  <>
