...
import {
  GraduationCap, TrendingUp, Eye, EyeOff, NotebookPen, CalendarRange, Timer,
  CalendarClock, ChevronDown, ChevronUp, SlidersHorizontal, Play,
} from "lucide-react";
import {
  TASKS_KEY, FOCUS_SESSIONS_KEY, SCHEDULE_KEY, AGENDA_KEY, WIDGETS_KEY, SEED_WIDGETS,
  SEED_TASKS, SEED_FOCUS, SEED_SCHEDULE, SEED_AGENDA, GRADES, useLocalState,
  todayISO, daysBetween, formatLong, eventTarget, subjectIcon, type Course,
} from "@/lib/storage";

type WidgetId = "kpis" | "card" | "grades" | "homework" | "courses" | "focus" | "nextUp";
const WIDGET_LABELS: Record<WidgetId, string> = {
  kpis: "Indicateurs", card: "Carte étudiante", grades: "Évolution des notes",
  homework: "Devoirs", courses: "Cours du jour", focus: "Focus du jour", nextUp: "Prochain événement",
};
...
export default function Dashboard() {
  ...
  const [agenda] = useLocalState(AGENDA_KEY, SEED_AGENDA);
  ...
  <button key={id} onClick={() => setWidgets((w) => ({ ...w, [id]: !w[id] } as typeof SEED_WIDGETS))}
          ...
          <article className="glass glass-hover rounded-[1.75rem] p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="mt-2 text-[2rem] font-semibold leading-none tracking-tight text-text tabular-nums">15,4<span className="text-lg text-subtle">/20</span></p>
              </div>
              <span className="icon-chip"><GraduationCap size={20} /></span>
            </div>
            <span className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-600"><TrendingUp size={13} /> +0,6 vs dernier trimestre</span>
          </article>
          <article className="glass glass-hover rounded-[1.75rem] p-6">
            <div className="flex items-start justify-between">
              <div>
            ...
            <Link href="/tasks" className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-sky-500/10 px-2.5 py-1 text-xs font-medium text-sky-600">Gérer mes devoirs →</Link>
          </article>
          <article className="glass glass-hover rounded-[1.75rem] p-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm font-medium text-subtle">Focus aujourd&apos;hui</p>
                <p className="mt-2 text-[2rem] font-semibold leading-none tracking-tight text-text tabular-nums">{focusMinToday}<span className="text-lg text-subtle"> min</span></p>
              </div>
              <span className="icon-chip"><Timer size={20} /></span>
            </div>
            <span className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-violet-500/10 px-2.5 py-1 text-xs font-medium text-violet-600">{focusToday.length} session{focusToday.length > 1 ? "s" : ""} · {Math.round(focusMinToday / 25)} pomodoro</span>
          </article>
        </section>
      )}
          ...
          {widgets.card && (
            <article className="glass rounded-[1.75rem] p-6 lg:col-span-2">
              <div className="flex items-center justify-between">
                ...
                <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-600"><span className="h-1.5 w-1.5 rounded-full bg-emerald-500" /> Validée</span>
              </div>
              <div className="relative mt-5 aspect-[8/5] overflow-hidden rounded-3xl bg-gradient-to-br from-amber-300 via-orange-400 to-rose-400 p-6 text-white shadow-xl shadow-orange-500/30">
                ...
                <div className="relative flex h-full flex-col justify-between">
                  ...
                  <div className="space-y-3">
                    ...
                    <p className="font-mono text-lg tracking-[0.12em] tabular-nums">{cardRevealed ? "2025 0847 2211" : "•••• •••• 2211"}</p>
                  </div>
                  <div className="flex items-end justify-between">
                    <div><p className="text-[9px] uppercase tracking-widest opacity-70">Élève</p><p className="text-sm font-semibold tracking-wide">OZAN</p></div>
                    <div className="text-right"><p className="text-[9px] uppercase tracking-widest opacity-70">Valide jusqu&apos;au</p><p className="text-sm font-semibold tracking-wide">{cardRevealed ? "06/2026" : "••/••••"}</p></div>
                  </div>
                </div>
              </div>
              <button onClick={() => setCardRevealed((v) => !v)} className="glass-inset glass-hover mt-5 flex w-full items-center justify-center gap-2 rounded-2xl py-3 text-sm font-medium text-text">
                {cardRevealed ? <EyeOff size={16} /> : <Eye size={16} />}{cardRevealed ? "Masquer les détails" : "Afficher les détails"}
              </button>
            </article>
          )}
          {widgets.grades && (
            <article className="glass rounded-[1.75rem] p-6 lg:col-span-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="glass-inset flex rounded-full p-1">
                  {["T1", "T2", "T3"].map((t) => (
                    <button key={t} onClick={() => setTerm(t)} className={`rounded-full px-3.5 py-1.5 text-xs font-medium transition-all duration-200 ${term === t ? "bg-accent text-white shadow" : "text-subtle hover:text-text"}`}>{t}</button>
                  ))}
                </div>
              </div>
      ...
      <section className="grid grid-cols-1 gap-4 sm:gap-5 lg:grid-cols-2">
                ...
                return (
                  <li key={t.id} className="flex items-center gap-3 rounded-2xl p-3 transition-colors hover:bg-white/50">
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="block text-xs text-subtle">{t.subject ?? "Sans matière"}{t.subtasks.length > 0 ? ` · ${t.subtasks.filter((s) => s.done).length}/${t.subtasks.length}` : ""}</span>
                    </span>
                    {t.due && (
                      <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-medium ${late ? "bg-rose-500/10 text-rose-600" : soon ? "bg-orange-500/10 text-orange-600" : "bg-black/5 text-subtle"}`}>
        ...
        {widgets.courses && (
          <article className="glass rounded-[1.75rem] p-6">
            ...
            <ul className="mt-5 space-y-1">
              {todayCourses.length === 0 && <p className="rounded-2xl bg-white/30 py-6 text-center text-sm text-subtle">{jsDay === 0 || jsDay === 6 ? "Week-end 🎉" : "Pas de cours aujourd'hui"}</p>}
          ...
          {widgets.focus && (
            <article className="glass rounded-[1.75rem] p-6">
              ...
              <div className="glass-inset mt-5 flex items-center justify-between rounded-2xl p-5">
                <div>
                  <p className="text-3xl font-bold tabular-nums text-text">{focusMinToday}<span className="text-base font-medium text-subtle"> min</span></p>
